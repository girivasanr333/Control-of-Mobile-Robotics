...
def update_state_probabilities(motion_probabilities,current,front_cell,front_z,front_s,visited_cells):
    ...
    maze1_prob.append(measure_probabilities);
...
def maze1_probabilities():
    ...
    maze1_cell_seq.append(10); 
    update_prob=update_state_probabilities(state_prob,'10', '9',0,0,visited_cells);
    visited_cells.update({'9':'x'});
    maze1_cell_seq.append(9); 
    #update_prob=update_state_probabilities(state_prob,'9', '0',1,1,visited_cells);

    update_prob=update_state_probabilities(state_prob,'9', '13',0,0,visited_cells); # make a right turn
    visited_cells.update({'13':'x'});
    maze1_cell_seq.append(13); 
    update_prob=update_state_probabilities(state_prob,'13', '14',0,0,visited_cells);
    visited_cells.update({'14':'x'});
    maze1_cell_seq.append(14); 
    update_prob=update_state_probabilities(state_prob,'14', '15',0,0,visited_cells);
    visited_cells.update({'15':'x'});
    maze1_cell_seq.append(15); 
    update_prob=update_state_probabilities(state_prob,'15', '16',0,0,visited_cells);
    visited_cells.update({'16':'x'});
    maze1_cell_seq.append(16); 

    print(" Maze1 probabilities for each move:")
    for each in maze1_prob:
        if each !=0:
            print(each);
    
    print("Maze1 cell sequence");
    print(maze1_cell_seq);
    return();
