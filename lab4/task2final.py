...
def get_next_direction(row, column, current_cell, current_direction, maze):
    ...
    if(cell_readings[direction_index] == "O"):
        d = current_direction
...
def localization( max_time=180, maze=maze1):
    # Initialization
    ...
    while (current_time - start_time) < max_time and not is_all_cells_visited():
        ...
        #update_state_probabilities(state_probabilities, r, r, current_cell)

        # Print the current state and probability
        #print(f"Current state: {r}, {c}")
        #print(f"State probability: {state_probabilities.get(current_cell, 0)}")
